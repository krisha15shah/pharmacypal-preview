...
import {
  Calculator, Search, ChevronDown, RotateCcw, BookOpen,
...
// ─── Calculator row (accordion item) ─────────────────────────────────────────
function CalcRow({ calc }: { calc: CalcMeta }) {
  const [open, setOpen] = useState(false);
  const Comp = CALC_COMPONENTS[calc.id];
  return (
    <div className="border-b border-slate-100 last:border-0">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-medium text-slate-800 truncate">{calc.name}</span>
          {calc.reference && (
            <span className="hidden sm:inline text-xs text-slate-400 truncate">· {calc.reference}</span>
          )}
        </div>
        <ChevronDown className={cn("w-4 h-4 text-slate-400 shrink-0 transition-transform ml-2", open && "rotate-180")} />
      </button>
      {open && (
        <div className="px-4 pb-4 pt-1 bg-slate-50 border-t border-slate-100">
          {calc.description && (
            <p className="text-xs text-slate-500 mb-3">{calc.description}</p>
          )}
          {Comp ? <Comp /> : <div className="text-slate-400 text-sm">Coming soon.</div>}
        </div>
      )}
    </div>
  );
...
// ─── Category block ───────────────────────────────────────────────────────────
function CalcCategoryBlock({ category, calcs }: { category: Category; calcs: CalcMeta[] }) {
    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden mb-2">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-slate-500">{CATEGORY_ICONS[category]}</span>
          <span className="text-xs font-bold uppercase tracking-wider text-slate-600">{category}</span>
          <span className="text-[10px] font-medium text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">{calcs.length}</span>
        </div>
        <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", open && "rotate-180")} />
      </button>
      {open && (
        <div className="divide-y divide-slate-100 border-t border-slate-100">
          {calcs.map(c => <CalcRow key={c.id} calc={c} />)}
        </div>
      )}
    </div>
  );
        ...
        <div className="max-w-2xl mx-auto px-4 py-5">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-slate-400">
              ...
              <div className="text-sm">No calculators match "{search}"</div>
            </div>
          ) : (
            <>
              {search && (
                <div className="text-xs text-slate-500 mb-3">{filtered.length} result{filtered.length !== 1 ? "s" : ""} for "{search}"</div>
              )}
              {Array.from(grouped.entries()).map(([cat, calcs]) => (
                <CalcCategoryBlock key={cat} category={cat} calcs={calcs} />
              ))}
            </>
          )}

          {/* Disclaimer */}
          <div className="mt-3 bg-slate-100 border border-slate-200 rounded-lg p-3 text-xs text-slate-500 flex items-start gap-2">
